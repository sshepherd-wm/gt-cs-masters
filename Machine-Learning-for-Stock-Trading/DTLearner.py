...
class DTLearner(object):                   
    ...
    def __init__(self, leaf_size=5, classification=True, verbose=False):                   
        """                   
        if classification = False, leaves hold means. if True, they hold modes.             
        """                   
    ...
    def find_best_feature(self, data):
        '''given a numpy array of labeled points where label is the first column,
        find feature with best absolute value corr'''
        tgt = data[:,0]
        max_abs_corr = 0
        best_feature = None
        for i in range(1, data.shape[1]): ## features start on second column
            feat = data[:,i]
            ## in case where feature or target values are all the same
            if len(set(feat)) == 1 or len(set(tgt)) == 1:
                if max_abs_corr == 0:
                    best_feature = i
                else:
                    continue

            abs_corr = np.abs(np.corrcoef(tgt,feat)[0][1])
            if (abs_corr is None) or (abs_corr > max_abs_corr):
                max_abs_corr = abs_corr
                best_feature = i
        
        return (best_feature, max_abs_corr)
    ...
    def build_tree(self, data, leaf_size=1):
        ...
        ## otherwise make sub-trees
            ...
            i = self.find_best_feature(data)[0]
